...
class Cropper:
    def __init__(self, cropping_models, ratio):
        self.net = cropping_models.net
        self.vgg_model = cropping_models.vgg_model
        self.sam_model = cropping_models.sam_model
        self.ratio = ratio
    ...
    def _translate_to_original(self, saliency, img, shape_r, shape_c):
        ...
        if rows_rate > cols_rate:
            ...
            y_end, x_end = (s_h, x_start+new_cols)
        else:
            new_rows = (h*s_w)//w
            y_start, x_start = ((s_h-new_rows)//2, 0)
            y_end, x_end = (y_start+new_rows, s_w)
    ...
    def _get_saliency(self, img):
        ...
        logging.info(f'saliency map shape: {saliency.shape}')
        saliency = self._translate_to_original(saliency, img, shape_r, shape_c)
        logging.info(f'saliency map shape(traslated into original size): {saliency.shape}')
        return saliency        


    def _get_crops(self, img, num_grid = 5, scales = [0.5, 0.6, 0.7, 0.8, 0.9, 1], 
        ...
        if crops_data['valid'].sum() > 0 & use_not_crossed:
            logging.info('return crop candidates with no face crossed and enough saliency')
            return crops_data[crops_data['valid']]['crop_coords'].tolist() 
        elif crops_data['not_crossed'].sum() > 0 & use_not_crossed:
            logging.info('return crop candidates with no faces crossed but not enough saliency')
            return crops_data[crops_data['not_crossed']]['crop_coords'].tolist()
        elif crops_data['salient'].sum() > 0:
            logging.info('return crop candidates with enough saliency but faces crossed.')
            return crops_data[crops_data['salient']]['crop_coords'].tolist()
        else:
            logging.info('return all candidates')
    ...
    def get_features(self, img):
        ...
        logging.info(f'The shape of crops(# of crops, height, width, channel):{ crops.shape}')
